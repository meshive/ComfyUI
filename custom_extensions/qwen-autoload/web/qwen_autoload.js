...
// Loads the bundled Qwen-Image workflow once per browser, only if the user
...
const FLAG = "qwen.autoloaded.v1";
const WORKFLOW_URL = new URL("./qwen_image.json", import.meta.url);
...
app.registerExtension({
    ...
    async setup() {
        ...
        try {
            ...
            if (!res.ok) {
                console.warn("[qwen] workflow fetch failed:", res.status);
                return;
            }
            const data = await res.json();
            await app.loadGraphData(data);
            localStorage.setItem(FLAG, "1");
            console.log("[qwen] auto-loaded Qwen-Image workflow");
        } catch (e) {
            console.warn("[qwen] auto-load error:", e);
        }
    },
});
